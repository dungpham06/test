...
- **Phân trang Frontend:** Hiện tại giao diện Frontend đang tải toàn bộ danh sách Todo một lần (`size: 10000`). Trong tương lai cần bổ sung UI phân trang (Pagination Bar hoặc Infinite Scroll).
- **Tính năng chia sẻ Todo (Todo Sharing):** Chưa hỗ trợ chia sẻ Todo giữa các người dùng với quyền Viewer/Editor. Nội dung này được phân tích chi tiết trong tài liệu thiết kế kỹ thuật **Tier 3A** (`docs/TODO_SHARING_SPEC.md`).
